...
const getGuardian = async () => {
   ...
   try{
    ...
    const data = response.data.response.results.map(article => (
        {source : article.type, id: article.pillarId , author : '' , title : article.webTitle , description : '' , url : article.webUrl, urlToImage : '' , publishedAt : article.webPublicationDate}
    ))
    return data
}
catch(error){
